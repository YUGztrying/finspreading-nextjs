...
export default function HorsBilanPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `/api/statements/list?user_id=${user.id}&statement_type=hors_bilan`
        )
  ...
  useEffect(() => {
    ...
    const fetchStatement = async () => {
      ...
      try {
        const response = await fetch(
          `/api/statements/list?user_id=${userId}&company_name=${encodeURIComponent(selectedCompany)}&statement_type=hors_bilan`
        )
        ...
        if (result.statements && result.statements.length > 0) {
          ...
          setNotification({
            type: 'info',
            message: `Aucun hors-bilan trouvé pour ${selectedCompany}`
          })
          setStatement(null)
        }
      } catch (error: any) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-stone-600">
                Visualisez et modifiez les engagements hors-bilan
              </p>
            </div>
          </div>
